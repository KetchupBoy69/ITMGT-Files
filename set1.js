...
function savings(grossPay, taxRate, expenses) {
    boogsh = grossPay-(grossPay*taxRate)-expenses
    return Math.floor(boogsh)
...
function materialWaste(totalMaterial, materialUnits, numJobs, jobConsumption) {
    bing = numJobs * jobConsumption 
    woosh = totalMaterial - bing
    ans = woosh + materialUnits
    return(ans) 
}
...
function interest(principal, rate, periods) {
    ...
    returns = (principal * rate) * periods + principal
    woogsh = Math.floor(returns)
    return(woogsh)
}
