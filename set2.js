...
function caesarCipher(message, shift) {
    ...
    for (x=0; x<i; x++) {
        if (message.charAt(x) === " "){
            ...
            y = ((message.charCodeAt(x)) - 65 + shift) % 26
            char += letters[y]
        }
    }
return(char)
...
function vigenereCipher(message, key) {
    ...
    for (x=0; x<j; x++){
        if (message.charAt(x) === " "){
            ...
            y = ((message.charCodeAt(x) - 65) + (key.charCodeAt(x%i) - 65))%26
            char += letters[y]
        }   
    }
    return(char)
...
function scytaleCipher(message, shift) {
    ...
    numc = (Math.ceil(i/shift))
    numf = (Math.floor(i/shift))
    ret = (i % shift) 
    ...
    for (x=0; x<numc; x++){
        for (y=0; y<shift; y++){    
            if (y<=ret-1){
                z = (y)*numc + (x)
                char += message.charAt(z) 
            }
            if (y > ret-1) {
                z = (ret)*numc + (y - ret - 1)*numf + (x)
                char += message.charAt(z) 
            }
        }
    }
...
function scytaleDecipher(message, shift) {
    let x = Math.ceil(message.length/shift)
    let char = ""
